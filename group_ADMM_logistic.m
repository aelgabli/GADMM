function [obj_GADMM, loss_GADMM, Iter]=group_ADMM_logistic(XX,YY, rho, no_workers, num_feature, per_samples, num_iter, obj0, lambda_logistic, acc)
...
%         end
        %cvx_solver mosek
        minimize(lambda_logistic*0.5*norm(x)^2+sum(log(1+exp(-YY(first:last).*(XX(first:last,1:s1)*x))))- C1'*x+C2'*x + term_1...
            +term_2)
