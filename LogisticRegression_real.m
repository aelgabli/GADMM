...
rho=0.03;
[obj_GADMM_rho0003, loss_GADMM_rho0003, Iter_0003] = group_ADMM_logistic(X_fede,y_fede, rho, num_workers, num_feature, per_split, num_iter, obj0, lambda, acc);
rho

rho=0.05;
[obj_GADMM_rho0005, loss_GADMM_rho0005, Iter_0005] = group_ADMM_logistic(X_fede,y_fede, rho, num_workers, num_feature, per_split, num_iter, obj0, lambda, acc);
rho


rho=0.07;
[obj_GADMM_rho0007, loss_GADMM_rho0007, Iter_0007] = group_ADMM_logistic(X_fede,y_fede, rho, num_workers, num_feature, per_split, num_iter, obj0, lambda, acc);
rho

% rho=0.09;
% [obj_GADMM_rho0009, loss_GADMM_rho0009, Iter_0009] = group_ADMM_logistic(X_fede,y_fede, rho, num_workers, num_feature, per_split, num_iter, obj0, lambda, acc);
% rho
...
save logReg_24Users_realData.mat obj_GD obj_cyclic_IAG obj_R_IAG obj_LAG_PS obj_LAG_WK...
    loss_GD loss_cyclic_IAG loss_R_IAG loss_LAG_PS loss_LAG_WK opt_obj...
    obj_GADMM_rho0003 loss_GADMM_rho0003 obj_GADMM_rho0005 loss_GADMM_rho0005 obj_GADMM_rho0007 loss_GADMM_rho0007...
...
legend('GD','cyclic-IAG','R-IAG','LAG-PS','LAG-WK','GADMM, \rho=3E-2'...
    ,'GADMM, \rho=5E-2','GADMM, \rho=7E-2');%,'Batch-GD')
%ylim([10^-4 10^3])
%xlim([10 30000])
...
ylabel('Objective Error','fontsize',16,'fontname','Times New Roman')
legend('GD','LAG-PS','LAG-WK','GADMM, \rho=3E-2'...
    ,'GADMM, \rho=5E-2','GADMM, \rho=7E-2');%,'Batch-GD')
%ylim([10^-4 10^3])
%xlim([200 100000])
set(gca,'fontsize',14,'fontweight','bold');
...
%semilogy(cumulative_com_LAG_WK, loss_LAG_WK,'m-','LineWidth',3);
%hold on
semilogy(cumulative_com_GADMM_rho0003,loss_GADMM_rho0003, 'k-','LineWidth',3);
hold on
semilogy(cumulative_com_GADMM_rho0005,loss_GADMM_rho0005, 'k--','LineWidth',3);
hold on
